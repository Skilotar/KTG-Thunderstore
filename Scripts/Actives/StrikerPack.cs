...
namespace Knives
{
    class Striker_Pack : PlayerItem
    {
        ...
        public static void Register()
        {
            ...
            string shortDesc = "Hold -Use- To Charge";
            string longDesc = "A particle acceleration pack. Those particles are typically of the user strapped into the harness. Can be used any time there is fuel in the meter. Running out of fuel will require a full reset." +
                "\n\n\n - Knife_to_a_Gunfight";
        ...
        public override void Update()
        {
            if (this.LastOwner != null)
            {
                if (this.IsCurrentlyActive)
                {
                    ...
                    if (Key(GungeonActions.GungeonActionType.UseItem) && KeyTime(GungeonActions.GungeonActionType.UseItem) > .1f && !locked)
                    {
        ...
        public static IEnumerator HandleMeterUse(PlayerItem item, float duration, PlayerController user, Action<PlayerController> OnFinish)
        {
            ...
            while (GetPrivateType<PlayerItem, float>(item, "m_activeElapsed") < GetPrivateType<PlayerItem, float>(item, "m_activeDuration") && item.IsCurrentlyActive)
            {
                ...
                if(UseHeld == false && KnownElaps >= 0)// regen fuel
                {
