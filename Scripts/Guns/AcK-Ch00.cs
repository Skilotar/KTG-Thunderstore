...
namespace Knives
{
    class Ack_Choo : AdvancedGunBehaviour
    {
        ...
        public static void Add()
        {
            ...
            gun.SetLongDescription("A weapon forged from a missunderstanding of the term pathogen bio weapon on the initial order. The blacksmith worked day and night to give the weapon life only to curse it with perpetual illness." +
                "\n\n\n - Knife_to_a_Gunfight"); ;
        ...
        private IEnumerator slightdelay(PlayerController player)
        {
            ...
            for (int i = 0; i < 6; i++)
            {
                ...
                if (flag2)
                {
                    ...
                    component.baseData.damage *= .5f * player.stats.GetStatValue(PlayerStats.StatType.Damage);
                    component.baseData.speed *= (1f - (Math.Abs(angle) / 40));
                    component.UpdateSpeed();
                    if(pois != 1)
                    {
                        component.PoisonApplyChance = 100f;
