    ...
    class FishBones : AdvancedGunBehaviour
    {
        ...
        private void OnHitEnemy(Projectile arg1, SpeculativeRigidbody arg2, bool fatal)
        {
            if (fatal)
            {
                ...
                if(frenzy == 1 && !Currentlyfrenzy)
                { 
                    StartCoroutine(FeedingFrenzy());
                   
                }
            }
        }
        public bool Currentlyfrenzy = false;
        public IEnumerator FeedingFrenzy()
        {
            Currentlyfrenzy = true;
