...
namespace Knives
{
	class HotShot : AdvancedGunBehaviour
	{
		...
		{
			Gun gun = ETGMod.Databases.Items.NewGun("Sizzler", "HotShot");
			...
			gun.SetupSprite(null, "HotShot_idle_001", 1);
			...
			gun.AddProjectileModuleFrom(PickupObjectDatabase.GetByEncounterName("Phoenix") as Gun, true, false);
			gun.GetComponent<tk2dSpriteAnimator>().GetClipByName(gun.chargeAnimation).wrapMode = tk2dSpriteAnimationClip.WrapMode.LoopSection;
			gun.GetComponent<tk2dSpriteAnimator>().GetClipByName(gun.chargeAnimation).loopStart = 3;

			Gun shellingFlash = (Gun)PickupObjectDatabase.GetByEncounterName("Phoenix");
