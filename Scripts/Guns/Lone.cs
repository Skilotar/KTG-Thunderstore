...
namespace Knives
{
    class Lone : AdvancedGunBehaviour
    {
        public static void Add()
        {
            ...
            gun.SetLongDescription("A single load lever action rifle used by the chidren of the sun to shoot stars out of the sky. " +
                
                "Reloads one bullet at a time. Can fire anytime a bullet is loaded. \n" +
                "Kills create stars that will burst and amp the bullet when shot." +
        ...
        public override void  Update()
        {
            ...
            if (gun.CurrentOwner)
            {
                ...
                if (!this.gun.IsReloading)
                {
                    ...
                    {
                        gun.reloadTime = ((this.gun.ClipCapacity - this.gun.ClipShotsRemaining) * .5f)/2;

                    }
                    else
                    {
                        gun.reloadTime = (this.gun.ClipCapacity - this.gun.ClipShotsRemaining) * .5f;
                    }
        ...
        public IEnumerator gradualReload()
        {
            ...
            {
                yield return new WaitForSeconds(1 * player.stats.GetStatValue(PlayerStats.StatType.ReloadSpeed));
            }

            yield return new WaitForSeconds(.5f * player.stats.GetStatValue(PlayerStats.StatType.ReloadSpeed));
