...
namespace Knives
{
    class Superstar : AdvancedGunBehaviour
    {
        public static void Add()
        {
            ...
            gun.SetLongDescription("A single load pump action shotgun used by the chidren of the sun during their duals over territory. " +

                "Reloads one shell at a time. Can fire anytime a bullet is loaded. \n" +
        ...
        public override void  Update()
        {
            ...
            if (gun.CurrentOwner)
            {
                ...
                if(gun.ClipShotsRemaining <=0 && Key(GungeonActions.GungeonActionType.Shoot, this.gun.CurrentOwner as PlayerController))
                {
                    ...
                    gun.reloadTime = ((this.gun.ClipCapacity - this.gun.ClipShotsRemaining) * .6f) * player.stats.GetStatValue(PlayerStats.StatType.ReloadSpeed);
        ...
        public IEnumerator gradualReload()
        {
            ...
            yield return new WaitForSeconds(.6f * player.stats.GetStatValue(PlayerStats.StatType.ReloadSpeed));
