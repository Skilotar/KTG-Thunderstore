...
namespace Knives
{
    ...
    public class Undercover : AdvancedGunBehaviour
    {
        public static void Add()
        {
            ...
            gun.SetLongDescription("A Gun designed to aid Undercover cops by being as inconspicuous as possible. The rounds it fires can penetrate any defense by pretending to be completely non threatening." +
                "\n\n" +
                "Bullets penetrate shielding moves, tables, doors, internal walls, chests, and boss dps cap" +
                "\n\n\n - Knife_to_a_Gunfight");
            ...
            if (otherbody != null && otherbody.aiActor != null)
            {
                if (otherbody.healthHaver != null)
                {
                    if (otherbody.healthHaver.GetCurrentHealth() > 0)
                    {
                        ...
                        otherbody.healthHaver.ApplyDamage(5 * user.stats.GetStatValue(PlayerStats.StatType.Damage), Vector2.zero, "Undercover", CoreDamageTypes.None, DamageCategory.Unstoppable, true, null, true);
                    }
